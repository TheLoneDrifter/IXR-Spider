...
import com.voltaccept.spideranimation.spider.configuration.LerpGait
import com.voltaccept.spideranimation.spider.configuration.SpiderDebugOptions
...
import com.voltaccept.spideranimation.utilities.isOnGround
import com.voltaccept.spideranimation.utilities.raycastGround
import com.voltaccept.spideranimation.utilities.resolveCollision
...
import com.voltaccept.spideranimation.utilities.maths.UP_VECTOR
import com.voltaccept.spideranimation.utilities.maths.lerp
import com.voltaccept.spideranimation.utilities.maths.pitch
...
import com.voltaccept.spideranimation.utilities.overloads.sendDebugChatMessage
...
class SpiderBody(
    ...
    fun refuel(amount: Int) {
        fuel = (fuel + amount).coerceAtMost(maxFuel)
    }
...
//            if (leg.isOutsideTriggerZone) return leg.endEffector
    ...
    fun update(ecs: ECS, entity: ECSEntity) {
        ...
        // Decrease fuel every minute (60,000 ms)
        val currentTime = System.currentTimeMillis()
        if (currentTime - lastFuelDecreaseTime >= 60000) {
            consumeFuel(1)
...
fun setupSpiderBody(app: ECS) {
    app.onTick {
        for ((entity, spider) in app.query<ECSEntity, SpiderBody>()) {
            ...
            // Remove dead spider after 2 seconds (now based on fuel)
            if (spider.fuel <= 0 && spider.deathTime > 0 && System.currentTimeMillis() - spider.deathTime > 2000) {
                if (!spider.isDisabled) {
